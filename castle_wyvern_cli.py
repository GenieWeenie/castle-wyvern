...
"""
    ...
    def route_and_respond_json(self, user_input: str, agent: str = None) -> dict:
        """Route input and return JSON response for scripting."""
        try:
            if agent and agent in self.clan:
                # Route to specific agent
                response = f"Response from {agent}: {user_input}"
                return {
                    "success": True,
                    "agent": agent,
                    "input": user_input,
                ...
                # Use intent router
                result = self.intent_router.classify(user_input)
                return {
                    "success": True,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  wyvern                          # Start interactive mode
  wyvern ask "How do I write a Python function?"
  wyvern code "function to calculate fibonacci"
        ...
        if select.select([sys.stdin], [], [], 0.0)[0]:
            ...
            if stdin_input:
                ...
                if args.json:
                    ...
                    result = cli.route_and_respond_json(stdin_input)
