...
class AlertRule:
    ...
    def __init__(self, name: str, metric_name: str, condition: Callable[[float], bool],
                 severity: AlertSeverity, message: str, cooldown_minutes: int = 5):
        self.name = name
...
class MonitoringService:
    ...
    def _check_memory(self) -> HealthStatus:
        ...
        try:
            ...
            if mem.percent > 95:
                ...
                status = "healthy"
                message = f"Memory OK: {mem.percent:.1f}% used ({mem.available / (1024**3):.1f} GB free)"
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "health_checks": {name: status.to_dict() 
                            for name, status in self.health.last_results.items()},
