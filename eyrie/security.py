...
class AuditLogger:
    ...
    def log(self, level: AuditLevel, category: str, action: str,
            user: str = "system", details: Dict = None,
            source_ip: str = "localhost", session_id: str = ""):
        """Log an audit event."""
        entry = AuditEntry(
    ...
    def log_config_change(self, user: str, config_key: str, old_value: Any, 
                         new_value: Any):
...
class EncryptionManager:
    ...
    def encrypt(self, data: str) -> str:
        ...
        encrypted = bytes(a ^ b for a, b in zip(data_bytes, self.key * (len(data_bytes) // len(self.key) + 1)))
        return encrypted.hex()
    
    def decrypt(self, encrypted_hex: str) -> str:
        """Decrypt hex-encoded data."""
        encrypted = bytes.fromhex(encrypted_hex)
        decrypted = bytes(a ^ b for a, b in zip(encrypted, self.key * (len(encrypted) // len(self.key) + 1)))
    ...
    def _check_default_keys(self) -> List[Dict]:
        ...
        for var in sensitive_vars:
            ...
            if value and len(value) < 20:  # Likely default/test key
                issues.append({
                    "severity": "warning",
                    "message": f"Potentially weak {var} detected"
                })
        ...
        for filepath in sensitive_files:
            if os.path.exists(filepath):
                ...
                if stat.st_mode & 0o044:
                    issues.append({
                        "severity": "error",
                        "message": f"{filepath} is readable by others"
                    })
...
class IntrusionDetector:
    ...
    def __init__(self, audit_logger: AuditLogger):
        ...
        self.suspicious_patterns = [
            ...
            ("unusual_access", lambda e: e.category == "data_access" and e.details.get("data_type") == "sensitive"),
            ("config_tampering", lambda e: e.category == "config" and e.level in ["warning", "error"]),
        ]
        self.thresholds = {
...
class SecurityManager:
    ...
    def validate_api_key(self, key: str, permission: str = None) -> bool:
        ...
        # Log the attempt
        self.log_auth("api", "api_key_validation", valid, 
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "recent_intrusions": len([d for d in self.intrusion_detector.detections 
...
if __name__ == "__main__":
    ...
    encrypted = security.encrypt_sensitive(secret)
    decrypted = security.decrypt_sensitive(encrypted)
    print(f"   Original: {secret[:10]}...")
    print(f"   Encrypted: {encrypted[:20]}...")
    print(f"   Decryption matches: {decrypted == secret}")
