...
class CommandCompleter(Completer):
    ...
    COMMANDS: List[str] = [
        # Basic commands
        "help", "status", "health", "members", "history", "memory",
        "quit", "exit", "bye",
        # Direct clan access
        ...
        "/history-search", "/history-clear", "/config",
        "/export", "/import",
        # Integrations
        "/integrations", "/slack-config", "/discord-config",
        "/slack-test", "/discord-test", "/alert",
        "/webhook-start", "/webhook-stop",
        # Security
        "/security-status", "/audit-log", "/audit-search", "/audit-export",
        "/apikey-create", "/apikey-list", "/apikey-revoke",
        "/security-scan", "/intrusion-check",
        # Enhanced Memory
        "/memory-add", "/memory-search", "/memory-context",
        "/memory-stats", "/memory-consolidate",
        # Browser Agent
        "/browse", "/research", "/browser-history", "/browser-clear",
        # Clan Creator
        "/clan-create", "/clan-create-confirm", "/clan-create-cancel",
        # Docker Sandbox
        "/sandbox-status", "/sandbox-exec", "/sandbox-lang",
        "/sandbox-list", "/sandbox-cleanup",
        # Goal-Based Agent
        "/goal", "/goal-status", "/goal-list", "/goal-execute",
        # Function Builder
        "/function-create", "/function-list", "/function-packs",
        "/function-search", "/function-show",
        # llama.cpp
        "/llama-status", "/llama-models",
        # Clan Backstories
        "/backstory", "/personalities",
        # nanoGPT
        "/nanogpt-create", "/nanogpt-train", "/nanogpt-list", "/nanogpt-sample",
        # Knowledge Graph
        "/kg-status", "/kg-add-entity", "/kg-add-rel", "/kg-query",
        "/kg-reason", "/kg-extract", "/kg-visualize", "/kg-export",
        "/kg-path", "/kg-report",
        # Visual Automation
        "/visual-status", "/visual-scan", "/visual-click", "/visual-type",
        "/visual-browser-start", "/visual-browser-task", "/visual-browser-end",
        "/visual-macro-login", "/visual-debug", "/visual-sessions", "/visual-replay",
        # Agent Coordination
        "/coord-status", "/coord-team", "/coord-run", "/coord-agents",
        "/coord-agent", "/coord-analytics", "/coord-optimize", "/coord-report",
        "/coord-compare",
        # Stretch Goals
        "/ai-optimize", "/ai-stats", "/perf-stats", "/perf-optimize",
        "/docs-generate", "/docs-export",
        # Visual Workflow Builder
        "/workflow-list", "/workflow-create", "/workflow-open", "/workflow-run",
        "/workflow-delete", "/workflow-export", "/workflow-import", "/workflow-template",
        # MCP Protocol
        "/mcp-start", "/mcp-stop", "/mcp-tools", "/mcp-install",
        # A2A Protocol
        "/a2a-start", "/a2a-stop", "/a2a-discover", "/a2a-agents", "/a2a-delegate",
    ]
    
    # Clan member names for tab completion
    CLAN_MEMBERS: List[str] = [
        "goliath", "lexington", "brooklyn", "broadway",
        "hudson", "bronx", "elisa", "xanatos", "demona", "jade"
    ]
    
    # Commands that accept member names as arguments
    MEMBER_COMMANDS: Set[str] = {
        "/clan", "/coord-agent", "/backstory", "/nanogpt-create"
    }
