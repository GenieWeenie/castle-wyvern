...
class CastleWyvern:
    ...
    def summon_council(self, user_input):
        ...
        # Technical Tasks
        if any(word in query for word in ["code", "script", "fix", "terminal", "bug", "error", "debug"]):
            agent, spell = "LEXINGTON", self.spells["lexington"]
        
        # Strategic Planning
        elif any(word in query for word in ["plan", "strategy", "choose", "versus", "options", "tactics"]):
            agent, spell = "BROOKLYN", self.spells["brooklyn"]
        
        # Summarization & Research
        elif any(word in query for word in ["summarize", "read", "explain", "info", "document", "analyze"]):
            agent, spell = "BROADWAY", self.spells["broadway"]
        
        # Archival & Historical Context
        elif any(word in query for word in ["history", "archive", "past", "remember", "before", "previous", "lesson"]):
            agent, spell = "HUDSON", self.spells["hudson"]
        
        # Security & Monitoring
        elif any(word in query for word in ["security", "monitor", "alert", "threat", "scan", "check", "safe", "protect"]):
            agent, spell = "BRONX", self.spells["bronx"]
        
        # Human Context & Ethics
        elif any(word in query for word in ["human", "legal", "law", "people", "social", "public", "ethics", "fair"]):
            agent, spell = "ELISA", self.spells["elisa"]
        
        # Red Team / Adversarial Testing
        elif any(word in query for word in ["test", "vulnerability", "exploit", "attack", "break", "hack", "weakness", "red team"]):
            agent, spell = "XANATOS", self.spells["xanatos"]
        
        # Failsafe / Worst-case
        elif any(word in query for word in ["fail", "worst", "disaster", "backup", "contingency", "what if", "could go wrong"]):
