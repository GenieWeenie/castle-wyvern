...
class DocumentIngestion:
    ...
    def _load_data(self):
        ...
        try:
            with open(self.docs_file, "r") as f:
                ...
                self.documents = {
                    str(d["id"]): Document(**d) for d in data.values()  # type: ignore[arg-type]
                }

            with open(self.chunks_file, "r") as f:
                data = json.load(f)
                self.chunks = {
                    str(c["id"]): DocumentChunk(**c)  # type: ignore[arg-type]
    ...
    def ingest_file(self, file_path: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        # Generate document ID (not for security - content addressing only)
        doc_id = hashlib.md5(
