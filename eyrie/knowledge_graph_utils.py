...
class KnowledgeGraphVisualizer:
    ...
    def to_mermaid(self) -> str:
        ...
        # Add relationships
        for rel_id, rel in self.kg.relationships.items():
            source = self.kg.entities[rel.source].name if rel.source in self.kg.entities else rel.source
            target = self.kg.entities[rel.target].name if rel.target in self.kg.entities else rel.target
...
class SmartEntityExtractor:
    ...
    def extract_rule_based(self, text: str) -> Dict:
        ...
        clan_members = ["goliath", "lexington", "brooklyn", "broadway", 
                       "hudson", "bronx", "elisa", "xanatos", "demona", "jade", "you"]
                ...
                if action in relation_map:
                    ...
                    if target_match:
                        extracted['relationships'].append({
                            'source': person.title() if person.lower() in clan_members else 'Someone',
