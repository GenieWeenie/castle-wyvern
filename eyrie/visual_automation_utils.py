        ...
        for session_file in self.storage_dir.glob("session_*.json"):
            with open(session_file, 'r') as f:
                ...
                sessions.append({
                    ...
                    'success_rate': data['success_count'] / (data['success_count'] + data['failure_count']) if (data['success_count'] + data['failure_count']) > 0 else 0
                })
...
class VisualMacro:
    ...
    def login_sequence(self, username: str, password: str, username_field: str = "username", password_field: str = "password", submit_button: str = "submit") -> Dict:
        """
    ...
    def form_fill_sequence(self, field_values: Dict[str, str], submit_button: str = "submit") -> Dict:
        """
        ...
        for field_desc, value in field_values.items():
            ...
            results['steps'].append({'step': step, 'action': f'click_{field_desc}', 'result': result})
            step += 1
            ...
            results['steps'].append({'step': step, 'action': f'type_{field_desc}', 'result': result})
            step += 1
