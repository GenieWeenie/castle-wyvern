...
class ResponseCache:
    ...
    def set(self, prompt: str, value: Any, model: str = "default", ttl_seconds: int = None):
        """Cache a response."""
        key = self._generate_key(prompt, model)
        ...
        with self._lock:
            ...
            self.cache[key] = CacheEntry(
                key=key, value=value, timestamp=time.time(), ttl_seconds=ttl_seconds
            )
    ...
    def invalidate(self, pattern: str = None):
        ...
        with self._lock:
            if pattern is None:
                ...
                keys_to_remove = [k for k in self.cache.keys() if pattern in self.cache[k].value]
...
class PerformanceManager:
    ...
    def get_cached_or_compute(
        self, prompt: str, compute_fn: Callable, model: str = "default"
    ) -> Any:
    ...
    def optimize_memory(self):
        ...
        return {"cache_cleared": True, "pools_cleared": True}
...
if __name__ == "__main__":
    ...
    # First call (miss)
    result = perf.get_cached_or_compute(prompt, lambda: "Python is a programming language.")
    print(f"   First call: {result[:30]}...")

    # Second call (hit)
    result = perf.get_cached_or_compute(prompt, lambda: "This should not be called")
