...
class PhoenixGate:
    ...
    def call_ai(self, prompt: str, system_message: str, mode: str = "cloud") -> str:
        ...
        if mode == "cloud":
            return str(self._call_with_fallback(prompt, system_message))
        elif mode == "local":
            return str(self._call_local(prompt, system_message))
        elif mode == "openai":
            return str(self._call_openai(prompt, system_message))
        else:
    ...
    def _call_with_fallback(self, prompt: str, system_message: str) -> str:
        ...
        try:
            ...
            return str(self._call_zai(prompt, system_message))
        ...
        # All failed
        raise PhoenixGateError(
            "All AI providers failed", severity=ErrorSeverity.CRITICAL, details={"errors": errors}
        )

    @retry_on_error(max_retries=3, delay=1.0, exceptions=(requests.exceptions.RequestException,))
    def _call_zai(self, prompt: str, system_message: str) -> str:
        ...
        result = zai_circuit_breaker.call(self._execute_zai_call, prompt, system_message)
        return str(result)
    ...
    def _execute_zai_call(self, prompt: str, system_message: str) -> str:
        ...
        if response.status_code == 401:
            raise PhoenixGateError(
                "Z.ai authentication failed - check API key", severity=ErrorSeverity.CRITICAL
            )
        elif response.status_code == 429:
            raise PhoenixGateError(
                "Z.ai rate limit exceeded - please wait", severity=ErrorSeverity.HIGH
            )
        elif response.status_code >= 500:
            raise PhoenixGateError(
                f"Z.ai server error: {response.status_code}", severity=ErrorSeverity.HIGH
            )
        ...
        if not content:
            raise PhoenixGateError(
                "Empty response content from Z.ai", severity=ErrorSeverity.MEDIUM
            )
        ...
        if "usage" in data:
            usage = data["usage"]
            logger.info(
                f"Z.ai usage: {usage.get('prompt_tokens', 0)} in, "
    ...
    def _call_openai(self, prompt: str, system_message: str) -> str:
        ...
        result = openai_circuit_breaker.call(
    ...
    def _execute_openai_call(self, prompt: str, system_message: str, api_key: str) -> str:
        ...
        response = requests.post(
            self.openai_url, headers=headers, json=payload, timeout=self.default_timeout
        )
        response.raise_for_status()
        data: Dict[str, Any] = response.json()
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            return {"status": "ERROR", "message": "No API key configured", "providers": []}
        ...
        return {"status": status, "message": message, "model": self.model, "providers": providers}
