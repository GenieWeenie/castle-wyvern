...
class AgentCoordinationLoop:
    ...
    def match_phase(self, task_id: str) -> TeamComposition:
        ...
        team_size = min(
            self.team_size_max,
            max(self.team_size_min, len(agent_fitness))
        )
        
        selected_agents = [agent_id for agent_id, _ in agent_fitness[:team_size]]
        
        # Calculate team formation score
        formation_score = sum(fitness for _, fitness in agent_fitness[:team_size]) / team_size if team_size > 0 else 0
        
        # Estimate success rate and completion time
        avg_reliability = sum(self.agents[aid].reliability for aid in selected_agents) / len(selected_agents) if selected_agents else 0
        avg_speed = sum(self.agents[aid].speed for aid in selected_agents) / len(selected_agents) if selected_agents else 1
    ...
    def score_phase(self, task_id: str, manual_score: Optional[float] = None) -> Dict:
        ...
        for agent_id in task.assigned_agents:
            ...
            if agent:
                ...
                agent.performance_score = (agent.performance_score * agent.tasks_completed + performance_score) / (agent.tasks_completed + 1)
    ...
    def get_coordination_stats(self) -> Dict:
        ...
        return {
            ...
            'avg_team_size': sum(len(t.assigned_agents) for t in self.completed_tasks) / len(self.completed_tasks) if self.completed_tasks else 0
        }
...
class ClanCoordinationManager:
    ...
    def _register_clan_members(self):
        ...
        clan_members = [
            ...
            ('lexington', 'Lexington', ['coding', 'technical', 'implementation', 'debugging'], 'technician'),
            ('brooklyn', 'Brooklyn', ['architecture', 'planning', 'strategy', 'design'], 'strategist'),
            ('broadway', 'Broadway', ['documentation', 'writing', 'summarization', 'communication'], 'chronicler'),
