...
class A2ATask:
    ...
    @classmethod
    def create(cls, session_id: str, message: A2AMessage, task_id: str = None) -> "A2ATask":
        """Create a new task."""
        return cls(
...
class A2AServer:
    ...
    def _execute_task(self, task: A2ATask) -> A2ATask:
        ...
        response_message = A2AMessage(
            role=A2AMessageRole.AGENT.value, parts=[{"type": "text", "text": response_text}]
        )
        task.messages.append(response_message)

        # Add artifact
        artifact = A2AArtifact(name="response", parts=[{"type": "text", "text": response_text}])
    ...
    def _determine_skill(self, query: str) -> str:
        ...
        if any(
            word in query_lower
            for word in ["code", "function", "bug", "debug", "python", "javascript"]
        ):
            return "lexington"
...
class A2AClient:
    ...
    async def send_task(
        self, agent_url: str, message: str, session_id: str = None
    ) -> Optional[A2ATask]:
    ...
    def start_server(self, host: str = "0.0.0.0", port: int = 18795) -> bool:
        ...
        try:
            ...
            server_thread = threading.Thread(
                target=self.server.run, kwargs={"debug": False}, daemon=True
            )
            server_thread.start()
...
if __name__ == "__main__":
    ...
    # Test task creation
    print("\n2. Task Creation:")
    message = A2AMessage(role="user", parts=[{"type": "text", "text": "Write a Python function"}])
