...
class IntentRouter:
    ...
    def _compile_patterns(self):
        ...
        for intent, patterns in self.INTENT_PATTERNS.items():
            self.compiled_patterns[intent] = [
                re.compile(p, re.IGNORECASE | re.VERBOSE) for p in patterns
            ]
    ...
    def classify(self, user_input: str, context: Optional[str] = None) -> IntentMatch:
        ...
        if keyword_confidence >= 0.8:
            logger.info(
                f"Keyword classification: {keyword_intent.value} ({keyword_confidence:.2f})"
            )
        ...
        if self.use_ai:
            try:
                ...
                logger.info(
                    f"{method.capitalize()} classification: {final_intent.value} ({final_confidence:.2f})"
                )
...
Confidence should be 0.0 to 1.0 based on how certain you are."""
        ...
        try:
            response = self.phoenix_gate.call_ai(
                f'Classify this user input: "{user_input}"', system_prompt, mode="cloud"
            )
    ...
    def _create_match(self, intent: IntentType, confidence: float, method: str) -> IntentMatch:
        ...
        return IntentMatch(
            ...
            primary_agent=str(routing["primary"]),
            fallback_agents=list(routing["fallback"]),
