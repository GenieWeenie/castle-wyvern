...
class MemoryEntry:
    ...
    @classmethod
    def create(cls, user_input: str, agent_name: str, agent_response: str,
               intent: str = "unknown", importance: int = 3, 
               tags: List[str] = None, session_id: str = "default"):
        """Factory method to create a new memory entry."""
        entry_id = hashlib.md5(
            f"{datetime.now().isoformat()}{user_input}".encode()
        ).hexdigest()[:12]
...
class GrimoorumV2:
    ...
    def _load_data(self):
        ...
        try:
            with open(self.memory_file, 'r') as f:
                data = json.load(f)
                self.memories = {
                    m['id']: MemoryEntry(**m) for m in data
                }
            
            with open(self.threads_file, 'r') as f:
                data = json.load(f)
                self.threads = {
                    t['id']: ConversationThread(**t) for t in data
                }
    ...
    def record(self, user_input: str, agent_name: str, agent_response: str,
               intent: str = "unknown", importance: int = 3,
               tags: List[str] = None, thread_id: str = None,
    ...
    def create_thread(self, topic: str, session_id: str = "default") -> str:
        ...
        thread_id = hashlib.md5(
            f"{datetime.now().isoformat()}{topic}".encode()
        ).hexdigest()[:12]
    ...
    def get_context_for_agent(self, agent_name: str, limit: int = 5,
    ...
    def search_by_tag(self, tag: str, limit: int = 10) -> List[Dict]:
        ...
        results = [
            asdict(e) for e in self.memories.values()
    ...
    def get_thread(self, thread_id: str) -> Optional[Dict]:
        ...
        entries = [
            asdict(self.memories[eid]) 
            for eid in thread.entries 
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_breakdown": {
                agent: len(entries) 
                for agent, entries in self.agent_contexts.items()
            },
            "high_importance": len([
                e for e in self.memories.values() if e.importance >= 4
            ]),
            "storage_size_kb": round(
                (os.path.getsize(self.memory_file) if os.path.exists(self.memory_file) else 0) / 1024, 2
            )
        }
    
    def prune_old_memories(self, keep_days: int = 30, 
    ...
    def get_formatted_context(self, agent_name: str = None, 
                             limit: int = 5) -> str:
        """
