...
class KnowledgeGraph:
    ...
    def add_relationship(
        ...
        # Generate ID
        rel_id = f"rel_{hashlib.md5(f'{source_id}_{relation}_{target_id}'.encode()).hexdigest()[:16]}"
    ...
    def get_related_entities(self, entity_id: str, relation: str = None) -> List[Tuple[Entity, str, Entity]]:
        """
    ...
    def _reason_suggestion(self, person: str, topic: str) -> Dict:
        ...
        for rel in self.get_relationships_from(person_id):
            if rel.relation == "suggested":
                target = self.entities.get(rel.target)
                if target and (topic.lower() in target.name.lower() or 
                              topic.lower() in str(target.properties).lower()):
                    suggestions.append({
    ...
    def _reason_intersection(self, topic1: str, topic2: str) -> Dict:
        ...
        for project in projects:
            ...
            if (topic1.lower() in related_names or any(topic1.lower() in r for r in related_names)) and \
    ...
    def extract_from_text(self, text: str, source: str = "") -> Dict:
        ...
        clan_members = ["goliath", "lexington", "brooklyn", "broadway", 
