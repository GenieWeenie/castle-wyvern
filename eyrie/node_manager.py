...
class StoneNode:
    ...
    @classmethod
    def create(cls, name: str, host: str, port: int = 18790,
               capabilities: List[str] = None) -> "StoneNode":
...
class NodeManager:
    ...
    def _load_nodes(self):
        ...
        try:
            with open(nodes_file, 'r') as f:
                data = json.load(f)
                self.nodes = {k: StoneNode(**v) for k, v in data.items()}
        ...
        try:
            with open(tasks_file, 'r') as f:
                data = json.load(f)
                self.tasks = {k: DistributedTask(**v) for k, v in data.items()}
        except (FileNotFoundError, json.JSONDecodeError):
            self.tasks = {}
    ...
    def register_node(self, name: str, host: str, port: int = 18790,
    ...
    def select_best_node(self, required_capabilities: List[str] = None) -> Optional[str]:
        """
    ...
    def assign_task(self, task_id: str, node_id: str = None) -> bool:
        """
...
if __name__ == "__main__":
    ...
    local = manager.get_node(manager.local_node_id)
    ...
    task_id = manager.create_task(
        "ai_call",
        {"prompt": "Write a Python function", "model": "glm-4-plus"},
        priority=4
    )
